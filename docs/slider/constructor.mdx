...
- *linkTo* - (string|object) links slider to other control ([linkTo()](/api/dhtmlxslider_linkto))
...
- *disabled* - (boolean) if set to true, slider is initially inactive, to make it active call the [enable()](/api/dhtmlxslider_enable) method, false by default
...
- *visible* - (boolean) if set to false, slider is initially hidden and will be shown on calling the method [show()](/api/dhtmlxslider_show), true by default
