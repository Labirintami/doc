...
In short, the full set of [Slider configuration properties](/slider/configuring_slider) can be applied to the range slider. All the standard [Slider API]() is supported.
...
Note that the <strong>value</strong> option should have the *array* type and contain a pair of values for both thumbs.


###Get/set value

- The <strong>getValue()</strong> method returns an array with a pair of values for a slider in the range mode. 
- The <strong>setValue()</strong> method takes as a first parameter an array, respectively.
