...
In order to select options in ComboBox, use the [setValue()](/combo/api/combobox_setvalue_method) method. It takes one parameter:

- <strong>ids</strong> - (*string|number|array*) the ids of items from data collection that should be selected in ComboBox
...
To get the list of selected options, apply the [getValue()](/combo/api/combobox_getvalue_method) method. It returns ids of selected options either as string(s) (by default) or as an array of strings. 
In the latter case you need to pass the following parameter:

- <strong>asArray</strong>  - (*boolean*) true, to return ids as an array of strings
...
To disable/enable ComboBox, make use of the [disable()](/combo/api/combobox_disable_method)/[enable()](/combo/api/combobox_enable_method) methods:
...
To check if a combobox is disabled, call the [isDisabled()](/combo/api/combobox_isdisabled_method) method:
...
You can repaint ComboBox on a page, e.g. after changing its configuration, using the [paint()](/combo/api/combobox_paint_method) method:
...
It is possible to clear the ComboBox input from the selected value(s) with the help of the [clear()](/combo/api/combobox_clear_method) method:
...
When needed, you can set focus in the ComboBox input without opening the list of options. Use the [focus()](/combo/api/combobox_focus_method) method for this purpose:
...
You can manipulate the visibility of the Combo popup with the help of the [Popup](/popup/api/refs/popup) API.
...
https://snippet.dhtmlx.com/ztc4sko8	Combobox. Popup
...
https://snippet.dhtmlx.com/ll0uykpk	Combobox. Popup Events

Using Data Collection API
------------------------

You can manipulate ComboBox options with the help of the [Data collection API](/data_collection/api/refs/datacollection). 

### Adding options into ComboBox

It is possible to add more options into the initialized ComboBox on the fly. Use the <strong>add()</strong> method of [Data Collection](/data_collection/api/refs/datacollection). It takes two parameters:
...
https://snippet.dhtmlx.com/o7vd331z	Combobox. Add/Remove

### Updating ComboBox options 

You can change config options of the option via the <strong>update()</strong> method of Data Collection. It takes two parameters:
...
https://snippet.dhtmlx.com/y7hpq0x8	Combobox. Update

### Removing options from ComboBox

To remove an option, make use of the <strong>remove()</strong> method of Data Collection. Pass the id of the option that should be removed to the method:
...
:::note
Check the full list of [Data collection API](/data_collection/api/refs/datacollection)1
:::
