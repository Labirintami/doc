...
You can [enable checkbox for tree items](/tree/configuration#checkboxesforitems) with the tree/api/tree_checkbox_config.md configuration option. Then you can manipulate items with checkboxes with the help of Tree API.
...
You can get/set the state of a tree using the Tree API - tree/api/tree_setstate_method.md and tree/api/tree_getstate_method.md. <strong>getState()</strong> returns an object with the state of a tree, while <strong>setState()</strong> takes an
...
You can manipulate Tree items with the help of the [Tree collection API](/tree_collection/api/refs/treecollection).

### Adding items into Tree

It is possible to add more items into the initialized Tree on the fly. Use the <strong>add()</strong> method of Tree Collection. It takes three parameters:
...
https://snippet.dhtmlx.com/hugcdfo9	Tree. Adding Data

### Updating Tree items

You can change config options of the item via the <strong>update()</strong> method of Tree Collection. It takes two parameters:
...
https://snippet.dhtmlx.com/e0vgry6n	Tree. Data Update

### Removing items from Tree 

To remove an item, make use of the <strong>remove()</strong> method of Tree Collection. Pass the id of the item that should be removed to the method:

~~~js
tree.data.remove("id");
~~~

https://snippet.dhtmlx.com/efy72vz8	Tree. Delete Data
