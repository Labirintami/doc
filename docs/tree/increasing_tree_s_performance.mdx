...
-  [Dynamic Loading](/tree/increasing_tree_s_performance#dynamicloading);
-  [Smart XML Parsing](/tree/increasing_tree_s_performance#smartxmlparsing);
-  [Distributed Parsing](/tree/increasing_tree_s_performance#distributedparsing).
...
-  <strong>id</strong> - (default) some_script?id=[item_id]
-  <strong>function</strong> - used for calling a user-defined handler that should be set as a parameter of the setXMLAutoLoading() method (see example below)
-  <strong>name</strong> - some_script[item_id]
-  <strong>xmlname</strong>  - some_script[item_id].xml
...
-  <strong>default</strong> - uses native escape functionality of the browser;
-  <strong>utf8</strong> - is used for UTF escaping;
-  <strong>none</strong> - data is not escaped.
...
-  <strong>mode</strong> - (true/false) enable/disable Distributed Parsing
-  <strong>count</strong> - number of nodes per portion (optional parameter);
-  <strong>delay</strong> - delay between portions parsing, set in milliseconds (optional parameter).
...
This method returns either <strong>true</strong> (meaning that parsing is still in progress) or <strong>false</strong> (meaning that parsing is already finished).
