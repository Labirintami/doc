...
Each <strong>data</strong> object can have the following properties:
...
You can disable displaying a checkbox for a tree item via the [update](/tree_collection/api/update) method of tree collection.

~~~js
tree.data.update("Books", {checkbox:false});
~~~

- <strong>parent</strong> - (*string*) the id of the parent of a tree item

For example, you can get the parent of an item using the [getItem](/tree_collection/api/getitem) method of tree collection.
