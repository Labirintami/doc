...
- specify the name of the created CSS class (or names of classes separated by spaces) as the value of the [css](/tree/api/tree_css_config) property in the Tree configuration:
...
You can change the default icons used for tree items. dhtmlxTree uses the [icons of the DHTMLX library](/helpers/icon) by default. However, you can use any other icon font pack, if necessary. 
...
Then you can use the name of the icon as the value of the <strong>icon</strong> property in the object with the configuration of an item. To use a new icon for a tree item, use the <strong>tree.data.update()</strong> method. Pass as parameters the
