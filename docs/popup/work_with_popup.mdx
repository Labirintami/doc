...
You can hide and show a popup using the [hide()](/popup/api/popup_hide_method) and [show()](/popup/api/popup_show_method) methods. The <strong>show()</strong> method takes the following parameters:
...
https://snippet.dhtmlx.com/aqzy536h	Popup. Show/Hide

Checking visibility of Popup
--------------

You can easily check whether a popup is visible with the help of the [isVisible()](/popup/api/popup_isvisible_method) method. It returns *true*, if a popup is visible.
...
You can easily attach some HTML content to a popup via the [attachHTML()](/popup/api/popup_attachhtml_method) method. It takes as a parameter a string with HTML.
...
dhtmlxPopup allows attaching other components of the library using the [attach()](/popup/api/popup_attach_method) method. It takes two parameters:
