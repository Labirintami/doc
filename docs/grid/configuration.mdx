...
import { Note, ProNote } from '@site/src/components';
...
<table class="webixdoc_links">
	<tbody>
        ...
        <tr>
            ...
            <li><b>rowspan</b> - (<i>number</i>) the number of rows in a rowspan <br/> https://snippet.dhtmlx.com/eol76o68	Grid. Header Spans</li>
        ...
        <tr>
			...
			<td>(<i>boolean</i>) if set to <i>true</i>, specifies the HTML content (inner HTML) of a column. If set to <i>false</i>, the content of the column cells will be displayed as a <i>string</i> value <br/> https://snippet.dhtmlx.com/chitkvkc	Grid. Html In Data</td>
		</tr>
        <tr>
			<td class="webixdoc_links0"><b>footer</b></td>
			<td>(<i>array</i>) an array of objects with footer rows configuration. Each footer object may include:
            <ul>
            <li><a href="https://docs.dhtmlx.com/suite/grid__customization.html#stylingfootercells"><b>text</b></a> - (<i>string|number</i>) the text of a footer <br/> https://snippet.dhtmlx.com/9jl55ep7	Grid. Grid With Footer</li>
            <li><b>colspan</b> - (<i>number</i>) the number of columns in a colspan</li>
        ...
        <tr>
			...
			<td>(<i>number</i>) the maximal width to be set for a column <br/> https://snippet.dhtmlx.com/ku3cfaux	Grid. Max Width</td>
		</tr>
        <tr>
			<td class="webixdoc_links0"><b>minWidth</b></td>
			<td>(<i>number</i>) the minimal width to be set for a column <br/> https://snippet.dhtmlx.com/x5hmpi9d	Grid. Min Width</td>
		</tr>
        <tr>
            ...
            <ol>- <b>col</b> - (<i>object</i>) the config of a column (see the <b>columns</b> config) <br/>https://snippet.dhtmlx.com/buirf16n	Grid. Mark Cells</ol>
            </li></ul>
            </td>
		</tr>
        <tr>
        ...
        <tr>
            ...
            <ul>
                ...
                <li><b>col</b> - (<i>object</i>) the config of a column (see the <b>columns</b> config)<br/>https://snippet.dhtmlx.com/9txizaow	Grid. Cell Templates</li>
            </ul>
            </td>
		</tr>
        <tr>
			<td class="webixdoc_links0"><a href="https://docs.dhtmlx.com/suite/grid__configuration.html#hiddencolumns"><b>hidden</b></a></td>
			<td>(<i>boolean</i>) defines whether a column is hidden <br/>https://snippet.dhtmlx.com/lh7ma639	Grid. Hidden Columns</td>
		</tr>
        <tr>
...
Starting from v6.5, there is the ability to align data in a column as well as to align data in the column's header via the <strong>align</strong> option:
...
https://snippet.dhtmlx.com/zfrpe22d	Grid. Adjust Columns
...
:::note
In case complex HTML content is added into a column, the column width may be calculated incorrectly.1
:::
...
You can specify data for your grid before initialization via the [data](/grid/api/grid_data_config) configuration property. There are also API methods for loading data into grid on the fly. Check the details in the article
...
You can disable this functionality for a separate column via the <strong>draggable</strong> configuration option of the column:
...
dhtmlxGrid supports drag-n-drop of rows between grids in several modes. To begin with, you should specify the [dragMode](/grid/api/grid_dragmode_config) property in the configuration object of Grid. Then define which mode you need:
...
https://snippet.dhtmlx.com/qx9a86ax	Grid. Setup Drag Mode
...
You can specify the way of editing the cells of a Grid column depending on its content as simple input, select control, date picker, checkbox or combobox. The type of the used editor is defined by the <strong>editorType</strong> property of a [column](/grid/api/grid_columns_config).
There are five types of column editors:

- <strong>input</strong> - an editor for cells with a simple text (the default one, unless a column has <strong>type:"date"</strong>)
...
- <strong>datePicker</strong> - an editor for cells with dates (default for a column with <strong>type:"date"</strong>)

To use this editor, you should specify the <strong>type:"date"</strong> property for a column. It is also possible to set the necessary [format of date](/calendar/api/calendar_dateformat_config) while editing a cell content 
with the help of the <strong>dateFormat</strong> option.
...
- <strong>select</strong> - an editor for cells that should contain several options to choose from

To set this editor type you need to provide the <strong>options</strong> property with an array of options to be displayed in the editor, e.g.:
...
- <strong>checkbox</strong> - an editor for cells with a two-state check box

To use this editor, you need to specify the <strong>type: "boolean"</strong> property for a column.
...
:::note
If you specify the editing option in the configuration of Grid, then editing of a column with checkbox will always be enabled.1
:::

- <strong>combobox</strong> - an editor for cells that should contain several options to choose from. There is a possibility to find an option by entering text in the edit control

To use this editor you need to specify the <strong>editorType: "combobox"</strong> property for a column and provide the <strong>options</strong> property with an array of options to be displayed in the editor, e.g.:
...
There is the [leftSplit](/grid/api/grid_leftsplit_config) property that splits grid columns into the frozen and movable parts. Just set the number of columns (from the left side of the grid) you want to freeze 
...
There are three types of filters that you can specify in the header/footer content of a [Grid column](/grid/api/grid_columns_config):

- <strong>inputFilter</strong> - provides a way of filtering data of a Grid column by using a text field
...
https://snippet.dhtmlx.com/4qz8ng3c	Grid. Header Filter

- <strong>selectFilter</strong> - allows end users to filter data of a column by choosing an option from a presented dropdown list
...
- <strong>comboFilter</strong> - provides a way to filter data of a column by choosing an option from a presented dropdown list. To find an option quickly you can enter text into the edit control.
...
If you specify <strong>comboFilter</strong> as the header or footer content of a column, you can set an additional config with properties for it.
...
You can set the <strong>hidden:true</strong> property in the [config of a column](/grid/configuration#columns) so that it doesn't appear on a page.
...
https://snippet.dhtmlx.com/lh7ma639	Grid. Hidden Columns
...
If you want to add custom elements into cells of the specified column, you need to set the <strong>htmlEnable:true</strong> property in the configuration of a column:
...
###<strong>Default shortcut keys</strong>
...
###<strong>Arrow shortcut keys</strong>
In case you want to enable the arrow keys that allow moving the selection between cells, you need to specify the [selection](/grid/api/grid_selection_config) property for Grid.
...
The arrow shortcut keys listed below do not work when the <strong>selection</strong> property is set to *"complex"*. Use another mode (*"cell" or "row"*) in case you you want to activate these navigation keys:
...
### <strong>Shortcut keys for editing</strong>

It is also possible to use shortcut keys for editing a cell in Grid by setting [editable:true](/grid/api/grid_editable_config) property in the configuration object of Grid.
...
While setting the [selection](/grid/configuration#selection) property either to *"row"* or to *"cell"* value, you can enable the grid/api/grid_multiselection_config.md property to allow a user to select multiple Grid elements:
...
Since the <strong>multiselection</strong> configuration option is set to *true*, using the "Ctrl + Click" combination allows selecting the desired cells or rows.
...
:::note
If you also set the <strong>autoWidth</strong> configuration option, you will be able to change the width of columns only inside the container of Grid. 1
:::
...
:::note
To define the resizing limits, set necessary values to the <strong>minWidth</strong>/<strong>maxWidth</strong> properties in the config of a column.
1
:::
...
There is a possibility to apply some styling to a row via the [rowCss](/grid/api/grid_rowcss_config) property. It is a function that takes the id of a row as a parameter and returns a string with the name of a CSS class.
...
dhtmlxGrid includes the selection feature that allows highlighting Grid elements depending on the chosen mode. The [selection](/grid/api/grid_selection_config) property enables selection in a grid. It can take three values:
...
The component has the [spans](/grid/api/grid_spans_config) property that allows you to specify all necessary columns and rows spans right through the initial configuration. It represents an array with spans objects.
...
https://snippet.dhtmlx.com/1775dwbl	Grid. Spans Cells

<strong>Note</strong>, that if both the <strong>spans</strong> and [leftSplit](/grid/configuration#frozencolumns) properties are set in the Grid config, the following rules will be applied:

- All necessary columns or rows will be in a span if the <strong>spans</strong> property is set for the columns located within the frozen area.
- If the <strong>spans</strong> property is set for a number of columns or rows placed as in the frozen part as in the movable one, then the columns remained in the movable part only will be in a span.
...
There is also the possibility to enable/disable tooltips for separate columns or spans by using the <strong>tooltip</strong> option in the configuration object of the [columns](/grid/configuration#columns) or [spans](/grid/configuration#spans) accordingly:
...
You can specify necessary size of your Grid via the configuration properties [width](/grid/api/grid_width_config) and [height](/grid/api/grid_height_config):
