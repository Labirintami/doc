...
You can specify data you want to add into TreeGrid on the initialization stage. Make use of the <strong>data</strong> configuration property, as in:
...
To load data from an external file, make use of the <strong>load</strong> method of Tree Collection. It takes the URL of the file with data as a parameter:
...
https://snippet.dhtmlx.com/44rmxlmq	TreeGrid.  External data loading
...
To load data from a local data source, use the <strong>parse</strong> method of Tree Collection. Pass [a predefined data set](#preparingdataset) as a parameter of this method:
...
To save the current state of a treegrid, use the <strong>serialize</strong> method of Tree Collection. It converts the data of a treegrid into an array of JSON objects. 
