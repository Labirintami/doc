...
You can specify the configuration of TreeGrid columns on the fly via the [setColumns()](/treegrid/api/treegrid_setcolumns_method) method. It takes an array with columns objects as a parameter.
...
You can find the full list of the available configuration options of a TreeGrid column [here](/treegrid/configuration#columns).      

### Getting configuration of a column

It is possible to return an object with attributes of a column via its id. Use the [getColumn()](/treegrid/api/treegrid_getcolumn_method) method for this purpose.
...
:::note
The [editEnd()](/treegrid/api/treegrid_editend_method) method does not work if [the type of the column editor](/treegrid/configuration#columneditors) is defined as <strong>checkbox</strong>.1
:::
