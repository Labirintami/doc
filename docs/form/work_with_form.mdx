...
- <strong>asFormData</strong> - (*boolean*) defines whether values of Form controls should be returned as Form Data
...
You can easily specify that an input is obligatory to be treated by a user with the help of the <strong>required</strong> attribute. 
...
While you've set <strong>required:true</strong> for a field, it gets an asterisk next to its label:

![Required fields](../assets/form/required_fields.png)

https://snippet.dhtmlx.com/0pr3var0	Form. Required
...
You just need to specify the minimum and/or maximum values allowed in the input via the <strong>min</strong> and/or <strong>max</strong> attributes.
...
For that, you need to use the <strong>minlength</strong> and (or) <strong>maxlength</strong> attributes that check the length of the given value. Validation is successful if the length is greater than or equal to the minlength value and (or) less than or equal to the maxlength value.
...
Set a string with the name of the necessary rule as a value of the <strong>validation</strong> attribute:
...
There is also a possibility to specify *a custom validation rule* by setting it as a value of the <strong>validation</strong> attribute:
...
https://snippet.dhtmlx.com/3cz9v7rm	Form. Validation

:::note
If the <b>inputType</b> attribute is set to "number", the <strong>validation</strong> attribute can be set only as a function.1
:::
...
https://snippet.dhtmlx.com/yhiuq2mi	Form. Messages
