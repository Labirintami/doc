...
A button can be easily added to a sidebar with the help of the <strong>add()</strong> method of Tree Collection:
...
A button can have an icon which is set through the corresponding option <strong>icon</strong>:
...
To hide/show a button, you should pass the ID of the button to the <strong>hide() / show()</strong> Sidebar methods:
...
The state of a two state button can be changed programmatically with the <strong>setState()</strong> method of Sidebar as in:
...
The current state of a two state button can be checked with the <strong>getState()</strong> method:
