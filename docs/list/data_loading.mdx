...
If the <strong>datatype</strong> parameter is skipped, the operation will try to process the incoming data as XML.
...
where <strong>some.json</strong> contains a value similar to the following one:
<br/><br/>
...
The names of the top level and the second level tags <strong>don't matter</strong>.
